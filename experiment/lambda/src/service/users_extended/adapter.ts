...
let handlerInstance: Promise<UsersExtendedHandler> | null = null;
...
function initializeHandler(): Promise<UsersExtendedHandler> {
  if (!handlerInstance) {
    handlerInstance = (async () => {
      const capabilities = await initializeCapabilities();
      return new UsersExtendedHandler(capabilities);
    })();
  }
