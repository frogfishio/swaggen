...
import { CreateUsersExtendedRequestBody, CreateUsersExtendedResponse } from "./proxy";
...
export class UsersExtendedHandler extends SwaggenHandler {
  ...
  public async createUsersExtended(req: SwaggenRequest): Promise<SwaggenResponse> {
    // Proxy method signature: createUsersExtended(data: CreateUsersExtendedRequestBody): Promise<CreateUsersExtendedResponse>;
    const data: CreateUsersExtendedRequestBody = req.body as CreateUsersExtendedRequestBody;

    const result = await this.proxy.createUsersExtended(data);
