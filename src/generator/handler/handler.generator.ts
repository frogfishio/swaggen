...
private generateProxyMethodSignature(
  ...
  if (["post", "put", "patch"].includes(method.toLowerCase()) && methodSpec.requestBody) {
    dataType = this.getRequestBodyType(method, endpoint, methodSpec.requestBody, usedTypes, schemaTypes);
  }
  ...
  if ("$ref" in requestBody) {
    ...
    if (content && content.schema) {
      if ("$ref" in content.schema) {
        ...
        // Generate a type name for the inline schema
        const typeName = `${toPascalCase(getMethodName(method, endpoint))}RequestBody`;
        usedTypes.add(typeName);
        // Optionally, you can handle the inline schema to generate the type definition
        return typeName;
      }
    }
  }
